use egui::{lerp, pos2, Color32, Painter, Pos2, Rect, Rgba, Shape, Stroke};
use luexks_reassembly::{blocks::shroud_layer::ShroudLayerColor, utility::display_oriented_math::DisplayOriented3D};

use crate::{selection_type::{self, SelectionType}, shroud_editor::ShroudEditor};
...
pub fn render_polygon(
    ...
    // let line_stroke = Stroke::new(line_width, shroud_editor.block_container.get_shroud_color(line_color));
    let line_stroke = Stroke::new(1.0, shroud_editor.block_container.get_shroud_color(line_color));
    ...
    (0..vertices.len())
        ...
        .for_each(|((vertex_a, vertex_b), vertex_c)| {
            ...
            // dbg!(&points);
            // painter.add(Shape::convex_polygon(points, fill_color, fill_stroke));
            painter.add(Shape::convex_polygon(points, fill_color, Stroke::NONE));
        });
    // painter.add(Shape::convex_polygon(vec![pos2(0.0, 0.0), pos2(1000.0, 1000.0), pos2(1000.0, 0.0)], fill_color, Stroke::NONE));
    // painter.line_segment([pos2(0.0, 0.0), pos2(1000.0, 1000.0)], stroke);
    (0..vertices.len())
        .map(|_| vertices[0])
    ...
    if let Some(selection_type) = selection_type_option {
        ...
        let avg_vert_pos = vertices.iter().fold(Pos2::default(), |pos, vertices| { pos2(pos.x + vertices.x, pos.y + vertices.y)}) / vertices.len() as f32;
        let selection_vertices = vertices.iter().map(|vertex| {
