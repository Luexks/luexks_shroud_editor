...
impl ShroudLayerInteraction {
    pub fn selection(&self) -> Vec<usize> {
        match self {
            ...
            ShroudLayerInteraction::Placing { selection, .. } => selection.clone(),
        }
    }
...
impl ShroudEditor {
    pub fn shroud_layer_interaction_update(
        &mut self,
        ui: &mut Ui,
        ctx: &Context,
        response: &Response,
        ...
        if let Some(mouse_pos) = mouse_pos {
            ...
            if let ShroudLayerInteraction::Placing { selection: _ } = &self.shroud_layer_interaction
            {
